...
To do that we only need to replace the webcam node that is sending the image. In this case the oasis agent is connected to the carla server and can send the current front ego vehicle camera within the simulator into the dataflow.
